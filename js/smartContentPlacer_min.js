...
a+"px";this.target.children.item(0).style.height=a+"px"}else{for(var f=document.createDocumentFragment(),g=function(){void 0!==this.renderDelay?this.elemCache[this.itteration].setAttribute("class","contentPlacerElement transition"):this.elemCache[this.itteration].setAttribute("class","contentPlacerElement");this.itteration++;this.itteration<this.elemCache.length&&setTimeout(g.bind(this),this.renderDelay)},b=0;b<a.length;){var e=this.generatePosition_(c,a[b].width,a[b].height),h=document.createElement("div");
h.className="contentPlacerElement";void 0!==this.renderDelay&&(h.className+=" hidden transition");h.style.width=a[b].width*d+(a[b].width-1)*this.margin+"px";h.style.height=a[b].height*d+(a[b].height-1)*this.margin+"px";h.style.top=e.top+"px";h.style.left=e.left+"px";h.innerHTML=a[b].content;f.appendChild(h);b++}a=this.rowsTot*d+(this.rowsTot-1)*this.margin;c=document.createElement("div");c.className="contentPlacerContainer";c.style.height=a+"px";c.appendChild(f);this.target.appendChild(c);this.target.style.height=
a+"px";this.target.style.minWidth=(d+this.margin)*this.largestElement+"px";this.elemCache=document.getElementsByClassName("contentPlacerElement");void 0!==this.renderDelay&&(this.itteration=0,setTimeout(g.bind(this),5));this.initiated=!0}};ContentPlacer.prototype.setMargin=function(a){this.margin=a;this.columns=Math.max(Math.floor(this.target.offsetWidth/(this.unitSize+this.margin)),this.largestElement);this.render_()};
ContentPlacer.prototype.setUnitSize=function(a){this.unitSize=a;this.columns=Math.max(Math.floor(this.target.offsetWidth/(this.unitSize+this.margin)),this.largestElement);this.render_()};ContentPlacer.prototype.setData=function(a){this.dataArray=a;this.findLargestNode_();this.calculateColumns_();this.target.innerHTML="";this.initiated=!1;this.render_()};ContentPlacer.prototype.setDelay=function(a){this.renderDelay=a};
