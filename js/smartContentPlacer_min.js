 ...
 */
var ContentPlacer=function(a,c,d,f,g){this.target=document.getElementById(a);this.unitSize=c;this.margin=d;this.data=f;this.rowsTot=1;this.initiated=!1;this.largestElement=1;this.renderDelay=g;this.itteration=0;this.elemCache=[];for(a=this.data.length;a--;)this.data[a].width>this.largestElement&&(this.largestElement=this.data[a].width);this.columns=Math.max(Math.floor(this.target.offsetWidth/(this.unitSize+this.margin)),this.largestElement);a=function(){var a=Math.max(Math.floor(this.target.offsetWidth/
(this.unitSize+this.margin)),this.largestElement);a!=this.columns&&(this.rowsTot=1,this.columns=a,this.render())}.bind(this);this.addEventHandler_(window,"resize",a);this.render()};ContentPlacer.prototype.matrix_=function(a,c,d){for(var f=[],g=0;g<a;g++){for(var b=[],e=0;e<c;e++)b.push(d);f.push(b)}return f};ContentPlacer.prototype.growMatrix_=function(a,c,d){for(var f=0;f<c;f++){for(var g=[],b=0;b<this.columns;b++)g.push(d);a.push(g)}return a};
ContentPlacer.prototype.addEventHandler_=function(a,c,d){a.addEventListener?a.addEventListener(c,d,!1):a.attachEvent&&a.attachEvent("on"+c,d)};
...
ContentPlacer.prototype.render=function(){var a=this.data,c=this.matrix_(this.rowsTot,this.columns,0),d=this.unitSize;if(this.initiated){for(b=0;b<a.length;)e=this.generatePosition_(c,a[b].width,a[b].height),this.elemCache[b].style.top=e.top+"px",this.elemCache[b].style.left=e.left+"px",b++;a=this.rowsTot*d+(this.rowsTot-1)*this.margin;this.target.style.height=a+"px";this.target.children.item(0).style.height=a+"px"}else{for(var f=document.createDocumentFragment(),g=function(){void 0!==this.renderDelay?
this.elemCache[this.itteration].setAttribute("class","contentPlacerElement transition"):this.elemCache[this.itteration].setAttribute("class","contentPlacerElement");this.itteration++;this.itteration<this.elemCache.length&&setTimeout(g.bind(this),this.renderDelay)},b=0;b<a.length;){var e=this.generatePosition_(c,a[b].width,a[b].height),h=document.createElement("div");h.className="contentPlacerElement";void 0!==this.renderDelay&&(h.className+=" hidden transition");h.style.width=a[b].width*d+(a[b].width-
1)*this.margin+"px";h.style.height=a[b].height*d+(a[b].height-1)*this.margin+"px";h.style.top=e.top+"px";h.style.left=e.left+"px";h.innerHTML=a[b].content;f.appendChild(h);b++}a=this.rowsTot*d+(this.rowsTot-1)*this.margin;c=document.createElement("div");c.className="contentPlacerContainer";c.style.height=a+"px";c.appendChild(f);this.target.appendChild(c);this.target.style.height=a+"px";this.target.style.minWidth=(d+this.margin)*this.largestElement+"px";this.elemCache=document.getElementsByClassName("contentPlacerElement");
void 0!==this.renderDelay&&(this.itteration=0,setTimeout(g.bind(this),5));this.initiated=!0}};
